...
export class InputType extends BaseComponent {
  node: HTMLElement;

  constructor(prop: IInputType) {
    super();
    this.node = this.build(prop);
  }
  build(prop: IInputType): HTMLElement {
    const input = this.createElem('div', 'relative h-8 w-full  min-w-[100px] max-w-auto');
    const inputLabel = this.createElem('label', 'w-fit h-min bg-white p-1 absolute left-2 -top-3 flex h-full w-full text-[11px] leading-tight text-stone-500 transition-all', prop.title);
